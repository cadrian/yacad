...
static yacad_event_t *event_provider(yacad_conf_t *conf) {
     // runs in the event queue thread
     return yacad_event_new(conf);
}
...
yacad_scheduler_t *yacad_scheduler_new(void) {
     ...
     result->event_queue = cad_new_event_queue_pthread(stdlib_memory, (provide_data_fn)event_provider, result->conf->get_event_queue_capacity(result->conf));
     result->event_queue->start(result->event_queue, result->conf);
     return &(result->fn);
}
